...
<?php
    ...
    $sql = "SELECT r.id, r.pid, p.fullname , r.quantity, (p.price - p.discount) AS product_price, (r.quantity * (p.price - p.discount)) AS total_amount, m.username, r.regdate FROM receipt r JOIN product p ON r.pid = p.id JOIN member m ON r.mid = m.id";
...
if(isset($_POST['submit'])){
    ...
    $mid = $_POST['mid'];
    $sql = "INSERT INTO `receipt`(`pid`, `quantity`, `mid`) VALUES ('$pid', '$quantity', '$mid')";
...
if(isset($_POST['update'])){
    ...
    $quantity = $_POST['quantity'];
    $mid = $_POST['mid'];
    $sql = "UPDATE receipt SET pid='$pid', quantity='$quantity', mid='$mid' WHERE id='$id'";
    mysqli_query($conn, $sql);
...
<body>
                ...
                <div class="displayData">
                    <table class="member display" id="member" style="width:100%">
                        ...
                        <?php while($receipt = mysqli_fetch_assoc($result)){ ?>
                                ...
                                <td><?php echo $receipt['regdate']; ?></td>
                                <td>
                                    <div class="action">
                                    <a href="?edit=<?php echo $receipt['id']; ?>" class="edit"><img src="img/icons/icons8_edit_24px_1.png" alt=""></a>
                                <a href="?delete=<?php echo $receipt['id']; ?>" class="delete"><img src="img/icons/icons8_trash_24px_1.png" alt=""></a>
                                    </div>
    ...
    <div class="crud" id="crud">
        ...
        <?php if (isset($_GET['edit'])){
            ...
            $sql1 = "SELECT `id`,  `pid`, `quantity`, `mid`, `regdate` FROM `receipt` WHERE `id` = $clicked_id";
            $result1 = mysqli_query($conn, $sql1);
            $receipt1 = mysqli_fetch_assoc($result1);
            }
            ?>
            <h3>Add / Edit</h3>
            <div class="group">
            ...
            <div class="group">
                <div class="part">
                    <p>Product ID</p>
                    <input type="text"  name="pid" value="<?php echo isset($_GET['edit']) && isset($receipt1['pid']) ? $receipt1['pid'] : ''; ?>">
                </div>
                <div class="part">
                    <p>Quantity</p>
                    <input type="text"  name="quantity" value="<?php echo isset($_GET['edit']) && isset($receipt1['quantity']) ? $receipt1['quantity'] : ''; ?>">
                </div>
                <div class="part">
                    <p>Member ID</p>
                    <input type="text"  name="mid" value="<?php echo isset($_GET['edit']) && isset($receipt1['mid']) ? $receipt1['mid'] : ''; ?>">
                </div>
            </div>
            <div class="actionBtn">
            <?php if (isset($_GET['edit'])) { ?>
